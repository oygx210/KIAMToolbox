module Translations
    ...
    contains
        ...
        subroutine kee2rv(ee, mu, grad_req, rv, drv) ! Vector and should be.
        ...
        subroutine krv2ee(rv, mu, grad_req, ee, dee) ! Vector and should be.
        ...
        subroutine koe2rv(oe, mu, grad_req, rv, drv) ! Vector and should be.
        ...
        subroutine krv2oe(rv, mu, grad_req, oe, doe) ! Vector and should be.
        ...
        subroutine kscrs2pa(XSCRS, JD, XPA, dXPA) ! Vector and should be.
		...
		subroutine kgcrs2scrs(XGCRS, JD, DistUnit, VelUnit, XSCRS)
		
			use Ephemeris
		
			implicit none
			
			real(dp), intent(in) :: XGCRS(:,:), JD(size(XGCRS,2))
			real(dp), intent(out) :: XSCRS(size(XGCRS,1),size(XGCRS,2))
			real(dp) :: xMoon(6), DistUnit, VelUnit
			integer :: i
			
			do i = 1,size(XGCRS,2)
				xMoon = PlanetState(JD(i),'Moon','Earth')
				xMoon(1:3) = xMoon(1:3)/DistUnit
				xMoon(4:6) = xMoon(4:6)/VelUnit
				XSCRS(:,i) = XGCRS(:,i) - xMoon
			end do		
		
		end subroutine kgcrs2scrs
		subroutine kscrs2gcrs(XSCRS, JD, DistUnit, VelUnit, XGCRS)
			...
			real(dp), intent(in) :: XSCRS(:,:), JD(size(XSCRS,2))
			real(dp), intent(out) :: XGCRS(size(XSCRS,1),size(XSCRS,2))
			real(dp) :: xEarth(6), DistUnit, VelUnit
			integer :: i
			
			do i = 1,size(XGCRS,2)
				xEarth = PlanetState(JD(i),'Earth','Moon')
				xEarth(1:3) = xEarth(1:3)/DistUnit
				xEarth(4:6) = xEarth(4:6)/VelUnit
				XGCRS(:,i) = XSCRS(:,i) - xEarth
			end do	
		
		end subroutine kscrs2gcrs
		subroutine kgcrs2hcrs(XGCRS, JD, DistUnit, VelUnit, XHCRS)
			...
			real(dp), intent(in) :: XGCRS(:,:), JD(size(XGCRS,2))
			real(dp), intent(out) :: XHCRS(size(XGCRS,1),size(XGCRS,2))
			real(dp) :: xSun(6), DistUnit, VelUnit
			integer :: i
			
			do i = 1,size(XGCRS,2)
				xSun = PlanetState(JD(i),'Sun','Earth')
				xSun(1:3) = xSun(1:3)/DistUnit
				xSun(4:6) = xSun(4:6)/VelUnit
				XHCRS(:,i) = XGCRS(:,i) - xSun
			end do
		
		end subroutine
		subroutine khcrs2gcrs(XHCRS, JD, DistUnit, VelUnit, XGCRS)
		
			use Ephemeris
		
			implicit none
			
			real(dp), intent(in) :: XHCRS(:,:), JD(size(XHCRS,2))
			real(dp), intent(out) :: XGCRS(size(XHCRS,1),size(XHCRS,2))
			real(dp) :: xEarth(6), DistUnit, VelUnit
			integer :: i
			
			do i = 1,size(XHCRS,2)
				xEarth = PlanetState(JD(i),'Earth','Sun')
				xEarth(1:3) = xEarth(1:3)/DistUnit
				xEarth(4:6) = xEarth(4:6)/VelUnit
				XGCRS(:,i) = XHCRS(:,i) - xEarth
			end do
		
		end subroutine
		subroutine kitrs2gcrs(XITRS, JD, XGCRS, dXGCRS) ! Vector and should be.
        ...
        subroutine kgcrs2itrs(XGCRS, JD, XITRS, dXITRS) ! Vector and should be.
