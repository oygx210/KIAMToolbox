module EquationsModule
	...
	! CR3BP and BR4BP parameters
	real(dp) :: MassParameter          ! mu
	real(dp) :: GravParameterFoursBody ! GM4b
	real(dp) :: DistanceToFoursBody    ! a4b
	real(dp) :: InitialSynodicPhase    ! theta0
    ...
    contains
        ...
        function knbp_ee_Moon(t,x) result(dxdt)
            ...
            real(dp) :: sinL, cosL, phi, eta, sigma, M(6,3), drv(6,6)

            call kee2rv(x, 1.0D0, .false., rv, drv)
            rvect = rv(1:3)
            vvect = rv(4:6)
		...
		function kr2bp(t,x) result(dxdt)
		...
		function kcr3bp(t,s) result(dsdt)
			...
			mu1 = 1.0D0 - MassParameter
			mu2 = MassParameter
            
			r12 = (s(1)+mu2)**2 + s(2)**2 + s(3)**2 ! r2
			r22 = (s(1)-mu1)**2 + s(2)**2 + s(3)**2 ! R2
            
			r13 = sqrt(r12)**3 ! r3
			r23 = sqrt(r22)**3 ! R3
            
			Ux = - s(1) + mu1*(s(1)+mu2)/r13 + mu2*(s(1)-mu1)/r23
			Uy = - s(2) + mu1* s(2)     /r13 + mu2* s(2)     /r23
			Uz =          mu1* s(3)     /r13 + mu2* s(3)     /r23

			dfdx = 0.0D0
			dsdt(1:3) = s(4:6)
			dsdt(4:6) = [ 2.0D0*s(5) - Ux, -2.0D0*s(4) -Uy, -Uz ]
			if (stm_required) then
				...
				Uxx = -1.0D0 + (mu1/r13)*(1.0D0-(3.0D0*(s(1)+mu2)**2/r12)) + &
  				               (mu2/r23)*(1.0D0-(3.0D0*(s(1)-mu1)**2/r22))
				
				Uyy = -1.0D0 + (mu1/r13)*(1.0D0-(3.0D0*s(2)**2/r12)) + &
				               (mu2/r23)*(1.0D0-(3.0D0*s(2)**2/r22))
	
				Uxy = -(mu1/r15)*3.0D0*s(2)*(s(1)+mu2) - (mu2/r25)*3.0D0*s(2)*(s(1)-mu1)
				Uxz =  3.0D0*mu2*s(3)*(mu1-s(1))/r25 - 3.0D0*mu1*s(3)*(mu2+s(1))/r15
				Uyz = -3.0D0*mu2*s(2)*s(3)/r25 -(3.0D0*mu1*s(2)*s(3))/r15
				
				Uzz = mu2/r23 + mu1/r13 - (3.0D0*mu2*s(3)**2)/r25 - (3.0D0*mu1*s(3)**2)/r15
				
				dfdx(1,4) = 1.0D0
				dfdx(2,5) = 1.0D0
				dfdx(3,6) = 1.0D0
				dfdx(4,:) = [-Uxx, -Uxy, -Uxz,  0.0D0, 2.0D0, 0.0D0]
				dfdx(5,:) = [-Uxy, -Uyy, -Uyz, -2.0D0, 0.0D0, 0.0D0]
				dfdx(6,:) = [-Uxz, -Uyz, -Uzz,  0.0D0, 0.0D0, 0.0D0]
        
				dPHIdt = dotmm(dfdx, PHI)
    
				dsdt(7:42) = reshape(dPHIdt, (/ 36 /))
    
			end if
		
		end function kcr3bp
		function kcr3bp_fb(t,s) result(dsdt)
			...
			r12 = (s(1))**2 + s(2)**2 + s(3)**2 ! r2
			r22 = (s(1)-1.0D0)**2 + s(2)**2 + s(3)**2 ! R2
            
			r13 = sqrt(r12)**3 ! r3
			r23 = sqrt(r22)**3 ! R3
            
			Ux = - s(1) + mu2 + mu1*(s(1))/r13 + mu2*(s(1)-1.0D0)/r23
			Uy = - s(2) + mu1* s(2)       /r13 + mu2* s(2)     /r23
			Uz =          mu1* s(3)       /r13 + mu2* s(3)     /r23
			...
			if (stm_required) then
				...
				Uxx = -1.0D0 + (mu1/r13)*(1.0D0-(3.0D0*(s(1))**2/r12)) + &
  				               (mu2/r23)*(1.0D0-(3.0D0*(s(1)-1.0D0)**2/r22))
				
				Uyy = -1.0D0 + (mu1/r13)*(1.0D0-(3.0D0*s(2)**2/r12)) + &
				               (mu2/r23)*(1.0D0-(3.0D0*s(2)**2/r22))
	
				Uxy = -(mu1/r15)*3.0D0*s(2)*(s(1)) - (mu2/r25)*3.0D0*s(2)*(s(1)-1.0D0)
				Uxz =  3.0D0*mu2*s(3)*(1.0D0-s(1))/r25 - 3.0D0*mu1*s(3)*(s(1))/r15
				Uyz = -3.0D0*mu2*s(2)*s(3)/r25 -(3.0D0*mu1*s(2)*s(3))/r15
		...
		end function kcr3bp_fb
		function kcr3bp_sb(t,s) result(dsdt)
			...
			r12 = (s(1)+1.0D0)**2 + s(2)**2 + s(3)**2 ! r2
			r22 = (s(1))**2 + s(2)**2 + s(3)**2 ! R2
            
			r13 = sqrt(r12)**3 ! r3
			r23 = sqrt(r22)**3 ! R3
            
			Ux = - s(1) - mu1 + mu1*(s(1)+1.0D0)/r13 + mu2*(s(1))/r23
			Uy = - s(2) + mu1* s(2)       /r13 + mu2* s(2)     /r23
			Uz =          mu1* s(3)       /r13 + mu2* s(3)     /r23
			...
			if (stm_required) then
				...
				Uxx = -1.0D0 + (mu1/r13)*(1.0D0-(3.0D0*(s(1)+1.0D0)**2/r12)) + &
  				               (mu2/r23)*(1.0D0-(3.0D0*(s(1))**2/r22))
				
				Uyy = -1.0D0 + (mu1/r13)*(1.0D0-(3.0D0*s(2)**2/r12)) + &
				               (mu2/r23)*(1.0D0-(3.0D0*s(2)**2/r22))
	
				Uxy = -(mu1/r15)*3.0D0*s(2)*(s(1)+1.0D0) - (mu2/r25)*3.0D0*s(2)*(s(1))
				Uxz =  3.0D0*mu2*s(3)*(-s(1))/r25 - 3.0D0*mu1*s(3)*(s(1)+1.0D0)/r15
				Uyz = -3.0D0*mu2*s(2)*s(3)/r25 -(3.0D0*mu1*s(2)*s(3))/r15
		...
		end function kcr3bp_sb
		function khill(t,s) result(dsdt)
		
			implicit none

			! khill  Уравнения Хилла.
			!
			!   Образует правую часть уравнений ограниченной задачи трех тел в
			!   приближении Хилла.
			!
			!   dsdt = khill(t,s) выдает правую часть уравнений орбитального движения в
			!   рамках ограниченной задачи трех тел в приближении Хилла. Время t игнорируется. 
			!   Вектор состояния s имеет размеры [6 x 1], первые три компоненты -- радиус-вектор, 
			!   вторые три -- вектор скорости аппарата.
			!
			!   Примеры:
			!       dsdt = khill(0,[1;0;0;0;1;0])
			!
			!   Литература:
			!       Дубошин Г.Н. Справочное руководство по небесной механике и
			!       астродинамике. М.: Наука. Глав. ред. физ.-мат. лит., 1976, С. 552
			!
			! Автор: Широбоков Максим, 31.05.2019

			real(dp) :: t, s(6), dsdt(6), r32

			r32 = sqrt(sum(s(1:3)*s(1:3)))**3

			dsdt(1:3) = s(4:6)
			dsdt(4) =  2.0D0*s(5) + 3.0D0*s(1) - s(1)/r32
			dsdt(5) = -2.0D0*s(4) - s(2)/r32
			dsdt(6) = -s(3) - s(3)/r32

		end function khill
		function kbr4bp(t,PS) result(dPSdt)
		
			! --------------------------------------------------------------
			! kbr4bp         Right-hand side for the equations of motion in the BR4BP model
			!
			! t              epoch
			! PS             (6x1 or 42x1) phase state vector in the rotating frame
			! mu             mass parameter of the interior CR3BP model
			! GM4b           scaled gravitational parameter of the 4th body
			! a4b            scaled distance from the CoM of the three-body system to the 4th body
			! theta0         initial value of the synodic phase
			! includeVarEqs  logical flag whether the variational equations are included or not
			!
			! dPSdt          time derivative of the phase state vector
			!
			! Authors: Sergey Trofimov (Matlab), Shirobokov Maksim (Fortran)
			! Date: Feb 25, 2020
			...
			r1pow2 = (PS(1)+d1)**2 + PS(2)**2 + PS(3)**2
			r2pow2 = (PS(1)-d2)**2 + PS(2)**2 + PS(3)**2
			r1pow3 = sqrt(r1pow2)**3
			r2pow3 = sqrt(r2pow2)**3

			! Synodic frequency
			omega4b = sqrt((1.0D0+GravParameterFoursBody)/DistanceToFoursBody**3) - 1.0D0

			! Synodic phase
			theta = InitialSynodicPhase + omega4b*t

			s = sin(theta)
			c = cos(theta)

			r4bpow2 = (PS(1)-DistanceToFoursBody*c)**2 + (PS(2)-DistanceToFoursBody*s)**2 + PS(3)**2
			r4bpow3 = sqrt(r4bpow2)**3
			a4bpow2 = DistanceToFoursBody**2

			! First-order derivatives of the effective potential
			Ux = PS(1) - d2*(PS(1)+d1)/r1pow3 - d1*(PS(1)-d2)/r2pow3 - GravParameterFoursBody*( (PS(1)-DistanceToFoursBody*c)/r4bpow3 + c/a4bpow2 )
			Uy = PS(2) - d2*PS(2)/r1pow3 - d1*PS(2)/r2pow3 - GravParameterFoursBody*((PS(2)-DistanceToFoursBody*s)/r4bpow3 + s/a4bpow2 )
			Uz = - d2*PS(3)/r1pow3 - d1*PS(3)/r2pow3 - GravParameterFoursBody*PS(3)/r4bpow3
			
			dPSdt(1:3) = PS(4:6)
			dPSdt(4:6) = [ 2*PS(5) + Ux, -2*PS(4) + Uy, Uz ]

			if (stm_required) then

				PHI = reshape(PS(7:42), (/6, 6/))
		
				r1pow5 = r1pow2*r1pow3
				r2pow5 = r2pow2*r2pow3
				r4bpow5 = sqrt(r4bpow2)**5
		
				! Second-order derivatives of the effective potential
				Uxx = 1.0D0 - (d2/r1pow3)*(1.0D0-3.0D0*(PS(1)+d1)**2/r1pow2) - &
							  (d1/r2pow3)*(1.0D0-3.0D0*(PS(1)-d2)**2/r2pow2) -  &
							  GravParameterFoursBody/r4bpow3*(1.0D0-3.0D0*(PS(1)-DistanceToFoursBody*c)**2/r4bpow2)
							  
				Uyy = 1.0D0 + 3.0D0*(d2/r1pow5+d1/r2pow5)*PS(2)**2 - d2/r1pow3 - d1/r2pow3 - &
							  GravParameterFoursBody/r4bpow3*(1.0D0-3.0D0*(PS(2)-DistanceToFoursBody*s)**2/r4bpow2)
			
				Uzz = 3.0D0*(d2/r1pow5+d1/r2pow5)*PS(3)**2 - d2/r1pow3 - d1/r2pow3 - &
					  GravParameterFoursBody/r4bpow3*(1.0D0-3.0D0*PS(3)**2/r4bpow2)
			
				Uxy = 3.0D0*(d2/r1pow5*(PS(1)+d1) + (d1/r2pow5)*(PS(1)-d2))*PS(2) + &
					  3.0D0*GravParameterFoursBody/r4bpow5*(PS(1)-DistanceToFoursBody*c)*(PS(2)-DistanceToFoursBody*s)
			
				Uxz = 3.0D0*(d2/r1pow5*(PS(1)+d1)+(d1/r2pow5)*(PS(1)-d2))*PS(3) + &
					  3.0D0*GravParameterFoursBody/r4bpow5*(PS(1)-DistanceToFoursBody*c)*PS(3)
					  
				Uyz = 3.0D0*(d2/r1pow5+d1/r2pow5)*PS(2)*PS(3) + &
					  3.0D0*GravParameterFoursBody/r4bpow5*(PS(2)-DistanceToFoursBody*s)*PS(3)
				...
				dPHIdt = dotmm(dfdx, PHI)
				dPSdt(7:42) = reshape(dPHIdt,(/36/))
				
			end if

		end function kbr4bp
		function kbr4bp_fb(t,PS) result(dPSdt)
		
			! --------------------------------------------------------------
			! kcr4bp_fb      Right-hand side for the equations of motion in the BR4BP model wrt first primary
			!
			! t              epoch
			! PS             (6x1 or 42x1) phase state vector in the rotating frame
			! mu             mass parameter of the interior CR3BP model
			...
			r1pow2 = (PS(1))**2 + PS(2)**2 + PS(3)**2
			r2pow2 = (PS(1)-1.0D0)**2 + PS(2)**2 + PS(3)**2
			r1pow3 = sqrt(r1pow2)**3
			r2pow3 = sqrt(r2pow2)**3
			...
			r4bpow2 = (PS(1)-d1-DistanceToFoursBody*c)**2 + (PS(2)-DistanceToFoursBody*s)**2 + PS(3)**2
			r4bpow3 = sqrt(r4bpow2)**3
			a4bpow2 = DistanceToFoursBody**2

			! First-order derivatives of the effective potential
			Ux = PS(1) - d1 - d2*(PS(1))/r1pow3 - d1*(PS(1)-1.0D0)/r2pow3 - GravParameterFoursBody*( (PS(1) - d1 - DistanceToFoursBody*c)/r4bpow3 + c/a4bpow2 )
			Uy = PS(2) - d2*PS(2)/r1pow3 - d1*PS(2)/r2pow3 - GravParameterFoursBody*((PS(2)-DistanceToFoursBody*s)/r4bpow3 + s/a4bpow2 )
			Uz = - d2*PS(3)/r1pow3 - d1*PS(3)/r2pow3 - GravParameterFoursBody*PS(3)/r4bpow3
			...
			if (stm_required) then
				...
				! Second-order derivatives of the effective potential
				Uxx = 1.0D0 - (d2/r1pow3)*(1.0D0-3.0D0*(PS(1))**2/r1pow2) - &
							  (d1/r2pow3)*(1.0D0-3.0D0*(PS(1)-1.0D0)**2/r2pow2) -  &
							  GravParameterFoursBody/r4bpow3*(1.0D0-3.0D0*(PS(1)-d1-DistanceToFoursBody*c)**2/r4bpow2)
				...
				Uxy = 3.0D0*(d2/r1pow5*(PS(1)) + (d1/r2pow5)*(PS(1)-1.0D0))*PS(2) + &
					  3.0D0*GravParameterFoursBody/r4bpow5*(PS(1)-d1-DistanceToFoursBody*c)*(PS(2)-DistanceToFoursBody*s)
			
				Uxz = 3.0D0*(d2/r1pow5*(PS(1))+(d1/r2pow5)*(PS(1)-1.0D0))*PS(3) + &
					  3.0D0*GravParameterFoursBody/r4bpow5*(PS(1)-d1-DistanceToFoursBody*c)*PS(3)
		...
		end function kbr4bp_fb
		function kbr4bp_sb(t,PS) result(dPSdt)
		
			! --------------------------------------------------------------
			! kcr4bp_sb      Right-hand side for the equations of motion in the BR4BP model wrt second primary
			!
			! t              epoch
			! PS             (6x1 or 42x1) phase state vector in the rotating frame
			! mu             mass parameter of the interior CR3BP model
			...
			r1pow2 = (PS(1)+1.0D0)**2 + PS(2)**2 + PS(3)**2
			r2pow2 = (PS(1))**2 + PS(2)**2 + PS(3)**2
			r1pow3 = sqrt(r1pow2)**3
			r2pow3 = sqrt(r2pow2)**3
			...
			r4bpow2 = (PS(1)+1.0D0-d1-DistanceToFoursBody*c)**2 + (PS(2)-DistanceToFoursBody*s)**2 + PS(3)**2
			r4bpow3 = sqrt(r4bpow2)**3
			a4bpow2 = DistanceToFoursBody**2

			! First-order derivatives of the effective potential
			Ux = PS(1) + 1.0D0 - d1 - d2*(PS(1)+1.0D0)/r1pow3 - d1*(PS(1))/r2pow3 - GravParameterFoursBody*( (PS(1) - d1 + 1.0D0 - DistanceToFoursBody*c)/r4bpow3 + c/a4bpow2 )
			Uy = PS(2) - d2*PS(2)/r1pow3 - d1*PS(2)/r2pow3 - GravParameterFoursBody*((PS(2)-DistanceToFoursBody*s)/r4bpow3 + s/a4bpow2 )
			Uz = - d2*PS(3)/r1pow3 - d1*PS(3)/r2pow3 - GravParameterFoursBody*PS(3)/r4bpow3
			...
			if (stm_required) then
				...
				! Second-order derivatives of the effective potential
				Uxx = 1.0D0 - (d2/r1pow3)*(1.0D0-3.0D0*(PS(1)+1.0D0)**2/r1pow2) - &
							  (d1/r2pow3)*(1.0D0-3.0D0*(PS(1))**2/r2pow2) -  &
							  GravParameterFoursBody/r4bpow3*(1.0D0-3.0D0*(PS(1)+1.0D0-d1-DistanceToFoursBody*c)**2/r4bpow2)
				...
				Uxy = 3.0D0*(d2/r1pow5*(PS(1)+1.0D0) + (d1/r2pow5)*(PS(1)))*PS(2) + &
					  3.0D0*GravParameterFoursBody/r4bpow5*(PS(1)+1.0D0-d1-DistanceToFoursBody*c)*(PS(2)-DistanceToFoursBody*s)
			
				Uxz = 3.0D0*(d2/r1pow5*(PS(1)+1.0D0)+(d1/r2pow5)*(PS(1)))*PS(3) + &
					  3.0D0*GravParameterFoursBody/r4bpow5*(PS(1)+1.0D0-d1-DistanceToFoursBody*c)*PS(3)
