...
def plot(x: numpy.ndarray, y: numpy.ndarray, fig: plotly.graph_objects.Figure = None, xlabel: str = 'x', ylabel: str = 'y', name: str = ''):
    """
...
def grid_on(fig: plotly.graph_objects.Figure):
    ...
    """
    if fig.data[0]['type'] == 'scatter':
        fig.update_xaxes(showgrid=True, gridcolor='Black', griddash='dot')
        fig.update_yaxes(showgrid=True, gridcolor='Black', griddash='dot')
        return fig
    ...
    if fig.data[0]['type'] == 'scatter3d':
        fig.update_layout(
            scene=dict(
                ...
                zaxis=dict(showgrid=True, gridcolor='grey', gridwidth=0.5)
            )
        )
        return fig

    raise 'grid_on is implemented only for scatter and scatter3d figures.'
...
def set_default_grid(fig: plotly.graph_objects.Figure):
    ...
    if fig.data[0]['type'] == 'scatter3d':
        fig.update_layout(
            scene=dict(
                xaxis=dict(backgroundcolor='white', showgrid=True, gridcolor='grey', gridwidth=0.5),
                yaxis=dict(backgroundcolor='white', showgrid=True, gridcolor='grey', gridwidth=0.5),
                zaxis=dict(backgroundcolor='white', showgrid=True, gridcolor='grey', gridwidth=0.5)
            )
        )
        return fig

    raise 'set_default_grid is implemented only for scatter and scatter3d figures.'
def axis_equal(fig: plotly.graph_objects.Figure):
    """
